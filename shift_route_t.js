...
// [Angel K-022 2026-07-13] シフト行の日付表記ゆれを正規化。日本語「N月N日」形式へ統一してからパースする。
//   CREA/C091パーサーは行頭の先頭数字を日と誤認するため、日抜け・スラッシュを補正しないと
//   誤ったセルへゴミ書込(例:「7/15」→day7)やベンリー誤反映が起きる(2026-07-13 前欠変更テストで実害)。
//   ①スラッシュ「M/D」→「M月D日」 (例「7/15 前欠」→「7月15日 前欠」)
//   ②日抜け「(N月)?N」→「(N月)?N日」 (例「7月17 20-24上」→「7月17日 20-24上」)
//   1行目(タグ行)は対象外。既に「日」付き・「N日(曜)」・時刻(20-24等)・休み/OFF は不変。
function normalizeAngelDateLines(text) {
  const lines = String(text).split(/\r?\n/);
  for (let i = 1; i < lines.length; i++) {
    lines[i] = lines[i].replace(/^(\d{1,2})\/(\d{1,2})(?=\s|$)/, "$1月$2日"); // ①スラッシュ
    lines[i] = lines[i].replace(/^((?:\d+月)?\d+)(?!日)(?=\s)/, "$1日");        // ②日抜け
  }
  return lines.join("\n");
}
