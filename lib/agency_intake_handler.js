...
// 電話番号のゼロ落ち防止（USER_ENTEREDでも文字列として保存）
function asText(v) {
  const s = sanitizeLine(v);
  return s ? `'${s}` : '';
}
...
function buildRow(d) {
  return [
    ...
    asText(d.agency_contact), sanitizeLine(d.agency_email),
    sanitizeLine(d.store_name), sanitizeLine(d.pref), sanitizeLine(d.area),
    asText(d.store_phone), sanitizeLine(d.store_email),
 ...
 */
async function sendIntakeMails(d) {
...
module.exports = { registerAgencyIntakeRoute, validateIntake, sanitizeLine, escapeHtml, buildRow, buildEmailText, saveIntakeToSheet, sendIntakeMails };
